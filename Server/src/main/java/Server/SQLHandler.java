...
public class SQLHandler {
    ...
    public static List<List<String>> getchat(String user, String friend){
        List<List<String>> str = new ArrayList<>();
        try {
            ResultSet rs = stmt.executeQuery("SELECT message, " +
                    "CASE WHEN u.id = messages.author_id THEN 1 ELSE 0 END " +
                    "FROM messages JOIN users AS u ON u.nickname = '" + user + "' " +
                    "JOIN users AS f ON f.nickname = '" + friend + "' " +
                    "WHERE  u.id = messages.author_id AND f.id = messages.address_id OR " +
                    "u.id = messages.address_id AND f.id = messages.author_id");
            while (rs.next()){
                List<String> o = new ArrayList<>();
                o.add(rs.getString(1)); o.add(rs.getString(2));
                str.add(o);
            }
    ...
    public static void addmessage(String author, String address, String text) throws SQLException {
        ...
        if (rs.next()){
            stmt.executeUpdate("INSERT INTO messages (author_id, address_id, message, is_read) VALUES ('" + rs.getString(1) + "', '" + rs.getString(2) + "', '" + text + "', 0);");
        }
    }
